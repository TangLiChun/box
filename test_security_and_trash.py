...
class FileManagerSecurityTests(unittest.TestCase):
    def setUp(self):
        ...
        self.original_initial_admin_password = os.environ.get('INITIAL_ADMIN_PASSWORD')
        ...
        self.app = app_module.create_app({
            ...
            'NOTES_FOLDER': str(self.notes),
            'TRASH_FOLDER': str(self.trash),
    ...
    def tearDown(self):
        ...
        if self.original_initial_admin_password is None:
            os.environ.pop('INITIAL_ADMIN_PASSWORD', None)
        else:
            os.environ['INITIAL_ADMIN_PASSWORD'] = self.original_initial_admin_password
    ...
    def test_admin_password_change_flag_is_set_on_bootstrap_and_cleared_after_password_change(self):
        with self.app.app_context():
            self.assertTrue(app_module.is_admin_password_change_required())

        response = self.client.post(
            '/admin/password',
            data={'user_id': '1', 'new_password': 'changed-password'},
        )
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.get_json()['success'])

        with self.app.app_context():
            self.assertFalse(app_module.is_admin_password_change_required())
